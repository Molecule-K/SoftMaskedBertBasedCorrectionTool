...
from softmasked_bert.utils import dump_json, get_abs_path
...
def proc_item(item, convertor):
    ...
    for mistake in root.xpath('/ESSAY/MISTAKE'):
        mistakes.append({
            ...
            mistake.get('id'),
            'location':
            int(mistake.get('location')) - 1,
            'wrong':
            convertor.convert(mistake.xpath('./WRONG/text()')[0].strip()),
            'correction':
            convertor.convert(mistake.xpath('./CORRECTION/text()')[0].strip())
        })
    ...
    for mistake in mistakes:
        if mistake['id'] not in rst_items.keys():
            rst_items[mistake['id']] = {
                'original_text': get_passages_by_id(passages, mistake['id']),
                'wrong_ids': [],
        ...
        if len(ori_text) == len(cor_text):
            if ori_text[mistake['location']] in mistake['wrong']:
                rst_items[mistake['id']]['wrong_ids'].append(
                    mistake['location'])
                wrong_char_idx = mistake['wrong'].index(
                    ori_text[mistake['location']])
    ...
    for k in rst_items.keys():
        if len(rst_items[k]['correct_text']) == len(
                rst_items[k]['original_text']):
...
def proc_confusion_item(item):
    ...
    for mistake in root.xpath('/SENTENCE/MISTAKE'):
        mistakes.append({
            'location':
            int(mistake.xpath('./LOCATION/text()')[0]) - 1,
            'wrong':
            mistake.xpath('./WRONG/text()')[0].strip(),
            'correction':
            mistake.xpath('./CORRECTION/text()')[0].strip()
        })
