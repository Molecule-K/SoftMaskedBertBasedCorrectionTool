...
class WarmupCosineAnnealingLR(_LRScheduler):
    ...
    def get_lr(self) -> List[float]:
        if self.last_epoch <= self.warmup_epochs:
            ...
            return [self.eta_min_lr + (base_lr - self.eta_min_lr) *(1 + math.cos(math.pi * (self.last_epoch - self.delay_iters) /(self.max_iters - self.delay_iters))) / 2for base_lr in self.base_lrs]
